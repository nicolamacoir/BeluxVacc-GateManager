...
async function get_all_gates_for_airport(airport) {
	const gate_list = await new Promise((resolve, reject) => {
		db.find({ airport }, { _id: 0, __v: 0 }).sort({ occupied: -1, apron: 1 }).exec((err, result) => {
			if (err) reject(err);
			resolve(result);
		});
	});
...
async function get_all_possible_gates_for(airport, ac, apron_input) {
	const [aprons, backup_aprons] = apron_input;

	if (airport === 'EBBR' && ac === 'A388') {
		const super_gates = ['233R', '322', '328'];
		const gate_list = await new Promise((resolve, reject) => {
			db.find({ airport, gate: { $in: super_gates }, occupied: false }, { _id: 0, __v: 0 }).sort({ apron: 1 }).exec((err, result) => {
				if (err) reject(err);
				resolve(result);
			});
		});
	...
	let gate_list = await new Promise((resolve, reject) => {
		db.find({ airport, apron: { $in: aprons }, occupied: false }, { _id: 0, __v: 0 }).sort({ apron: 1 }).exec((err, result) => {
			if (err) reject(err);
			resolve(result);
		});
	});
	...
	if (gate_list.length === 0) {
		gate_list = await new Promise((resolve, reject) => {
			db.find({ airport, apron: { $in: backup_aprons }, occupied: false }, { _id: 0, __v: 0 }).sort({ apron: 1 }).exec((err, result) => {
				if (err) reject(err);
				resolve(result);
			});
		});
...
async function get_all_assigned_gates() {
	const gate_list = await new Promise((resolve, reject) => {
		db.find({ occupied: true }, {
			_id: 0, __v: 0, apron: 0, latitude: 0, longitude: 0, occupied: 0,
		}).exec((err, result) => {
			if (err) reject(err);
			resolve(result);
		});
...
async function get_gate_for_gateid(airport, gate_id) {
	const gate_obj = await new Promise((resolve, reject) => {
		db.findOne({ airport, gate: gate_id }, { _id: 0, __v: 0 }, (err, result) => {
			if (err) reject(err);
			resolve(result);
		});
	});
...
async function get_gate_for_callsign(callsign) {
	const gate_obj = await new Promise((resolve, reject) => {
		db.findOne({ assigned_to: callsign }, {
			_id: 0, __v: 0, apron: 0, latitude: 0, longitude: 0,
		}, (err, result) => {
			if (err) reject(err);
			resolve(result);
		});
...
async function set_gate_to_callsign(airport, gate_id, callsign) {
	...
	const gate_obj = await get_gate_for_gateid(airport, gate_id);
	if (gate_obj === null) {
	...
	const result_obj = await new Promise((resolve, reject) => {
		db.update({ airport, gate: gate_id }, gate_obj, (err, result) => {
			if (err) reject(err);
			resolve(gate_obj);
		});
	});
...
async function set_gate_for(callsign, airport, gateid) {
	const old_gate_obj = await get_gate_for_callsign(callsign);
...
async function swap_gate_for(callsign, gateid) {
	...
	/* Only clear old gate if callsign already has a gate assigned */
	if (old_gate_obj !== null && old_gate_obj.occupied) {
		const tmp = old_gate_obj.airport;
		const result_obj = await clear_gate(old_gate_obj.airport, old_gate_obj.gate);
...
async function load_active_clients() {
	...
	const intresting_clients = await fetch('https://api.beluxvacc.org/belux-active-runways/vatsim-clients', {
		headers: {
...
async function process_clients(clients) {
	...
	for (const [key, client] of Object.entries(clients.pilots)) {
		...
		const { arrival } = client.flight_plan;
		const { departure } = client.flight_plan;
		...
		if (dep_airport_info) { flight_object.dep_airport.detailed = (`${dep_airport_info.name} (${dep_airport_info.country})`); } else { flight_object.dep_airport.detailed = 'Airport unknown'; }

		flight_object.arr_airport = {};
		flight_object.arr_airport.icao = client.flight_plan.arrival;
		if (arr_airport_info) { flight_object.arr_airport.detailed = (`${arr_airport_info.name} (${arr_airport_info.country})`); } else { flight_object.arr_airport.detailed = 'Airport unknown'; }

		flight_object.aircraft = {};
		flight_object.aircraft.icao = AC_code;
		if (ac_info) { flight_object.aircraft.detailed = ac_info.name; } else { flight_object.aircraft.detailed = 'AC type unknown'; }
		...
		if (on_dep_ground || on_arr_ground) {
			...
			if (closestGate === null) {
				...
				if (cur_gate_obj === null) {
					...
					monitored_clients[callsign] = 'AUTO-DEP';
				} else if ((cur_gate_obj.gate !== closestGate.gate) && (cur_gate_obj.airport !== closestGate.airport)) {
					// Free reservation
		...
		if (airports_of_interest.includes(client.flight_plan.arrival) && (!both_airports_of_intrest || both_airports_already_processed !== 'A')) {
			...
			let cur_gate = null;
			if (parseInt(ground_speed) < 50) { continue; }
			if (arr_distance < 150) {
...
function bookkeep_clients() {
	Object.keys(monitored_clients).forEach(async (key) => {
		let i; let
			found = false;
		for (i = 0; i < pilots_of_interest.length; i++) {
			if (pilots_of_interest[i].callsign === key) { found = true; }
		}
		if (!found) {
...
exports.list_all_valid_gates = async function (req, res) {
	const airport = (req.params.airport ? req.params.airport.toUpperCase() : 'EBBR');
	const { callsign } = req.body;
	const { origin } = req.body;
	const ac = req.body.aircraft;
...
exports.get_all_assigned_gates = async function (req, res) {
	...
	const grouped_by_airport = {};
	for (const key in gate_list) {
		const { airport } = gate_list[key];
		if (!grouped_by_airport[airport]) { grouped_by_airport[airport] = {}; }
		grouped_by_airport[airport][gate_list[key].assigned_to] = gate_list[key].gate;
	}
...
exports.set_random_gate = async function (req, res) {
	const { callsign } = req.body;
	const airport = req.body.airport.toUpperCase();
	const { origin } = req.body;
	const ac = req.body.aircraft;
...
exports.set_gate = async function (req, res) {
	const { callsign } = req.body;
	const requested_airport = req.body.airport;
	const requested_gateid = req.body.gate_id;
...
exports.get_active_pilots = function (req, res) {
	if (req.params.airport && pilots_of_interest !== null) {
		const airport = req.params.airport.toUpperCase();
		const filtered_clients = pilots_of_interest.filter((el) => (el.arr_airport && el.arr_airport.icao === airport && el.type === 'A') || (el.dep_airport && el.dep_airport.icao === airport && el.type === 'D'));
...
exports.get_active_controllers = function (req, res) {
	if (req.params.airport && controllers_of_interest !== null) {
		const airport = req.params.airport.toUpperCase();
		const filtered_clients = controllers_of_interest.filter((el) => relevant_controllers[airport].includes(el.callsign));
		res.json({ updated: last_updated, clients: filtered_clients });
	} else {
